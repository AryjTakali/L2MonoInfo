...
void enlever_animal(Animal **liste, Animal *animal)
{
  ...
  while (*liste){
    if ((*liste)->suivant == animal){ //on verifie si l'element suivant est celui recherché
      tmp = (*liste)->suivant; //si c'est l'element recherché on stocke l'adresse de l'element qui le suit 
      (*liste)->suivant = (*liste)->suivant->suivant; // une fois l'animal enlevé on lie la suite de la liste a l'element actuel
      free(tmp);
      return; 
    }
    *liste = (*liste)->suivant;
